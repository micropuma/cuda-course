...
__global__ void reduce(float *device_input, float *device_output) {
    // 计算每个快的索引位置
    float *device_data = device_input + blockIdx.x * blockDim.x;     // 每个block的起始位置

    // 支持shared memory
    __shared__ float shared_data[THREAD_PER_BLOCK];      // 共享内存
    shared_data[threadIdx.x] = device_data[threadIdx.x]; // 将数据拷贝到共享内存
    __syncthreads();                                     // 同步线程，确保所有线程都完成数据拷贝
    
    // 这一版本支持no divergence
    for (int i = 1; i <blockDim.x; i *=2) {
        // 判断当前线程是否参与计算
        if (threadIdx.x < (blockDim.x / (2*i))) {   // 只有前半区的thread参与计算
            int index = threadIdx.x * 2 * i; // 计算当前线程的索引位置
            shared_data[index] += shared_data[index + i]; // 规约   
        }
...
int main() {
    ...
    //========== 输出内存分配 ==============
    int block_num = N / THREAD_PER_BLOCK; // 计算块数
    float *host_output = (float *)malloc((N / THREAD_PER_BLOCK) * sizeof(float));       // 主机输出
    float *device_output; // 设备输出
    cudaMalloc((void **)&device_output, (N / THREAD_PER_BLOCK) * sizeof(float)); // 设备输出，注意传入二级指针
    float *res = (float *)malloc((N / THREAD_PER_BLOCK) * sizeof(float));                // 结果
    ...
    for (int i = 0; i < block_num; i++) {
        ...
        for (int j = 0; j < THREAD_PER_BLOCK; j++) {
            res[i] += host_input[i * THREAD_PER_BLOCK + j];
        }
    }
